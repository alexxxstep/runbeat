...
export function LoginPage() {
  ...
  const spotifyRequired = searchParams.get('spotify_required') === 'true';
  const {
    user,
    signInWithGoogle,
    signInWithSpotify,
        ...
        {spotifyRequired && user ? (
          <div className='mb-6 p-4 bg-yellow-50 dark:bg-yellow-900/20 border-l-4 border-yellow-400 rounded'>
            <p className='text-yellow-800 dark:text-yellow-200 text-sm font-medium mb-2'>
              Потрібна авторизація Spotify
            </p>
            <p className='text-yellow-700 dark:text-yellow-300 text-sm'>
              Для використання чату необхідно підключити Spotify акаунт
            </p>
          </div>
        ) : (
