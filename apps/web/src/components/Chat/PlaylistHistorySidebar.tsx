...
export function PlaylistHistorySidebar({
  ...
  const { playlists, loading, error, deletePlaylist, refresh } =
    usePlaylistHistory(userId);

  // Refresh when trigger changes (e.g., after new playlist generation)
  useEffect(() => {
    if (refreshTrigger && userId) {
      refresh();
    }
  }, [refreshTrigger, userId, refresh]);
  ...
  return (
    <div className='w-64 bg-white dark:bg-gray-800 border-r border-gray-200 dark:border-gray-700 flex flex-col h-full transition-all duration-300'>
      ...
      <div className='flex-1 overflow-y-auto p-4'>
        ...
        {error && (
          <div className='text-red-500 text-sm py-4'>Помилка завантаження</div>
        )}
