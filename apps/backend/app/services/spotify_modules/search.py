...
class SearchMixin:
        ...
        try:
            ...
            async def search_task(query: str):
                try:
                    ...
                    status_code = getattr(e, "http_status", None) or getattr(e, "code", None)
                    if status_code == 403:
                        is_403_error = True

                    if is_403_error:
                        logger.error(
                            f"403 Forbidden on search query '{query}' - Spotify app is in Development Mode"
                        )
                        # Don't return None, raise to propagate the error
                        raise Exception(
            ...
            filtered_tracks = [
                ...
                if t.get("tempo")
                and min_tempo <= t["tempo"] <= max_tempo
                and t.get("energy", 0) >= min_energy
            ]
            ...
            logger.info(
                f"Found {len(filtered_tracks)} tracks matching criteria from {len(tracks)} unique results."
            )

            # If filtering returns too few tracks, return the best unfiltered ones
            if not filtered_tracks:
                logger.warning(
                    "No tracks matched filtering criteria. Returning top unfiltered results."
                )
    ...
    def _build_search_queries(
        self, seed_genres: List[str], search_query: Optional[str]
    ) -> List[str]:
        ...
        for genre in genres:
            ...
            search_queries.extend(
                [
                    f"{genre_term} workout",
                    f"{genre_term} fitness",
                    f"{genre_term} energetic",
                    f"{genre_term} upbeat",
                    f"{genre_term} running",
                ]
            )
    ...
    async def search_track_by_name(
        ...
        try:
            ...
            search_results = sp.search(q=query, type="track", limit=limit, market="US")
