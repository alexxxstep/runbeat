    ...
    async def get_recommendations(
        ...
        try:
            ...
            if user_token:
                ...
                logger.debug(
                    "Using user-specific Spotify client for recommendations.")
            else:
                sp = spotipy.Spotify(
            ...
            try:
                if not user_token:
                    ...
                    if not token_info:
                        raise Exception(
                            "Spotify authentication failed: no access token received")
            ...
            seed_artists_list = list(
                seed_artists[:2]) if seed_artists else []
            seed_tracks_list = []
                        ...
                        for genre in seed_genres_list[:2]:
                            try:
                                genre_map = {
                                    ...
                                    "hip-hop": "hip hop", "country": "country music", "house": "house music",
                                    "techno": "techno music", "dance": "dance music",
                                }
            ...
            try:
                ...
                is_404_error = "404" in error_str or "not found" in error_str or "http status: 404" in error_str
                status_code = getattr(spotify_error, 'http_status', None) or getattr(spotify_error, 'code', None)
                if status_code == 404:
                    is_404_error = True
                ...
                if is_404_error:
                    ...
                    try:
                        return await self.get_tracks_by_search(
                            seed_genres=seed_genres_list, min_tempo=min_tempo, max_tempo=max_tempo,
                            target_energy=target_energy, limit=limit,
                        )
    ...
    async def get_recommendations_advanced(
        ...
        **kwargs
        ...
        try:
            ...
            if seed_tracks_list: rec_params["seed_tracks"] = seed_tracks_list
            if seed_artists_list: rec_params["seed_artists"] = seed_artists_list
            if seed_genres_list: rec_params["seed_genres"] = seed_genres_list
